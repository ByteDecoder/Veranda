...
namespace Sleipnir
{
    ...
    public class OdinNode<T>
    {
        ...
        private List<Action> _onEvaluation;
        ...
        public void LoadDrawingData(List<OdinNode<T>> graph, OdinGraph<T> __graph)
        {
            ...
            var onEvaluate = methods
                .Where(m => m.GetCustomAttributes(typeof(EvaluateAttribute), false).Length > 0)
        ...
        {
            foreach (var action in _onEvaluation)
                action.Invoke();
        }
        ...
        public void Remap()
        {
            ...
            foreach (var odinSlot in mapped)
            {
                if (Slots.All(k => k.Key.DeepReflectionPath != odinSlot.Item1.DeepReflectionPath))
                {
                    ...
                    if (odinSlot.Item2.Direction.IsInput()) slots.Add(new Slot(SlotDirection.Input, Node, 0));
                    if (odinSlot.Item2.Direction.IsOutput()) slots.Add(new Slot(SlotDirection.Output, Node, 0));
                    Slots.Add(odinSlot.Item1, slots.ToArray());
                }
            }
