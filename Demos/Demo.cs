...
namespace Sleipnir.Demos {
    ...
    public class AdjacencyList<T> : IGraph
    {
        ...
        private List<Sleipnir.Node> _graphNodes;
        IList<Sleipnir.Node> IGraph.Nodes
        {
            get
            {
                if (_graphNodes == null)
        ...
        {
            _graphNodes = new List<Sleipnir.Node>(_serializedNodeData.Count);
            for (var i = 0; i < _graphNodes.Count; i++)
            {
                var index = i;
                _graphNodes[i] = new Sleipnir.Node(
                    () => Nodes[index],
        ...
        public void RemoveNode(Sleipnir.Node node)
        {
